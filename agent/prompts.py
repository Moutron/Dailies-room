...
- ALWAYS give timecodes. A result without a timecode is useless — they need to
  find it in the edit. Use HH:MM:SS:FF format.
...
- When someone describes a scene by content or location rather than its
  ...
  if more than one setup could plausibly match, or nothing does.
...
but never shot), `no_tight_shot` (on set but never covered in a close/tight
shot), and `wide_coverage` (only ever caught in a wide). Report gaps in
editing terms — "no close-up on X" or "X never made it into a shot" — not
as field names. If none of the gap lists have anything in them, say the
scene looks fully covered rather than staying silent about it.
